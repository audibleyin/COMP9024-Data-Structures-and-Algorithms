...
int TaskScheduler(char *f1, char *f2, int m) {
    ...
    LinkedList *tasks = malloc(m * sizeof(LinkedList));
    for (int i = 0; i < m; i++) {
        tasks[i] = createLinkedList();
    }
    ...
    while (releasedHeaps->size > 0 || deadlineHeaps->size > 0) {
        ...
        if (minCoreValue + deadlineNode->Etime <= deadlineNode->Dline) {
            ...
            insertList(tasks[coreOfMinValue], deadlineNode->TaskName,
                       cores[coreOfMinValue], deadlineNode->Etime, deadlineNode->Rtime, deadlineNode->Dline);
    ...
    for (int i = 0; i < m; i++) {
        printf("Core%d\n", i);
        printDoubleLinkedListNext(tasks[i]);
